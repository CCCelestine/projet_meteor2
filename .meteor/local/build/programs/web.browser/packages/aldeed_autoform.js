...
Utility = {
  ...
  getFieldDefinition: function (ss, name) {
    ...
    if (!def) return;
    return _extends({}, def, def.type && def.type[0] || {});
  },
  ...
  function addHooksToList(hooksList, hooks, replace) {
    ...
    hooks.before && _.each(hooks.before, function () {
      function autoFormBeforeHooksEach(func, type) {
        if (typeof func !== 'function') {
          throw new Error('AutoForm before hook must be a function, not ' + _typeof(func));
        }
    ...
    hooks.after && _.each(hooks.after, function () {
      function autoFormAfterHooksEach(func, type) {
        if (typeof func !== 'function') {
          throw new Error('AutoForm after hook must be a function, not ' + _typeof(func));
        }
      ...
      function autoFormHooksEach(name) {
        if (hooks[name]) {
          if (typeof hooks[name] !== 'function') {
            throw new Error('AutoForm ' + name + ' hook must be a function, not ' + _typeof(hooks[name]));
          }
...
Template.afQuickField.helpers({
  isGroup: function () {
    function afQuickFieldIsGroup() {
      ...
      var isSubschema = _typeof(c.defs.type) === 'object' && c.defs.type._schema;
